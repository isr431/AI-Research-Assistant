...
├── search.py         # Brave Search client (web + image search with domain blocking)
├── sources.py        # Source registry, scoring, citation validation
├── fetch_pages.py    # Full-page excerpt fetching, browser profile rotation & retry
...
The test suite covers history persistence, concurrent writes, deterministic search ordering, image search ranking/deduplication with domain blocking, source scoring, page extraction, browser profile validation and retry logic, coverage helpers, provider aliases, and cancellation.
