...
_BLOCKED_IMAGE_DOMAINS = {"tiktok.com"}


def _is_blocked_image_domain(domain: str) -> bool:
    """Return True if *domain* belongs to a blocked image source."""
    lowered = domain.lower().strip(".")
    return any(
        lowered == blocked or lowered.endswith("." + blocked)
        for blocked in _BLOCKED_IMAGE_DOMAINS
    )
...
def brave_image_search(
    ...
    *,
    ...
    for rank, item in enumerate(raw_results, 1):
        ...
        source_domain = str(normalized.get("source_domain") or "")
        if _is_blocked_image_domain(source_domain):
            continue
